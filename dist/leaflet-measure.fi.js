!(function(e) {
        ...
        if ((this._finishMeasure(), e.length)) {
          ...
          1 === e.length
            ? ((t = L.circleMarker(e[0], this._symbols.getSymbol('resultPoint'))),
              (r = b({ model: L.extend({ messages: this.options.messages }, n) })))
            : 2 === e.length
  ...
  function(e, t, r) {
    function n(e, t, r) {
      ...
      var S = t.variable;
      S || (k = 'with (obj) {\n' + k + '\n}\n'),
        (k = (x ? k.replace(m, '') : k).replace(y, '$1').replace(g, '$1;')),
        (k =
          'function(' +
          (S || 'obj') +
          ') {\n' +
  ...
  function(e, t, r) {
    e.exports =
      '<div class=group> <p class="lastpoint heading">Last point</p> <p>{{ model.lastCoord.dms.y }} <span class=coorddivider>/</span> {{ model.lastCoord.dms.x }}</p> <p>{{ numberFormat(model.lastCoord.dd.y, 6) }} <span class=coorddivider>/</span> {{ numberFormat(model.lastCoord.dd.x, 6) }}</p> </div> <% if (model.pointCount > 1) { %> <div class=group> <p><span class=heading>Path distance</span> {{ model.lengthDisplay }}</p> </div> <% } %> <% if (model.pointCount > 2) { %> <div class=group> <p><span class=heading>Pinta-ala</span> {{ model.areaDisplay }}</p> </div> <% } %> ';
  },
  function(e, t) {
    e.exports =
      '<h3>{{ model.messages.measurement }}</h3> <p>{{ model.lastCoord.dms.y }} <span class=coorddivider>/</span> {{ model.lastCoord.dms.x }}</p> <p>{{ numberFormat(model.lastCoord.dd.y, 6) }} <span class=coorddivider>/</span> {{ numberFormat(model.lastCoord.dd.x, 6) }}</p> <ul class=tasks> <li><a href=# class="js-zoomto zoomto">{{ model.messages.centerOnLocation }}</a></li> <li><a href=# class="js-deletemarkup deletemarkup">{{ model.messages.deleteMeasurement }}</a></li> </ul> ';
  },
  function(e, t) {
    e.exports =
      '<h3>{{ model.messages.measurement }}</h3> <p>{{ model.lengthDisplay }}</p> <ul class=tasks> <li><a href=# class="js-zoomto zoomto">{{ model.messages.centerOnLocation }}</a></li> <li><a href=# class="js-deletemarkup deletemarkup">{{ model.messages.deleteMeasurement }}</a></li> </ul> ';
  },
  function(e, t) {
    e.exports =
      '<h3>{{ model.messages.measurement }}</h3> <p>{{ model.messages.area }} {{ model.areaDisplay }}</p> <p>{{ model.messages.perimeter }} {{ model.lengthDisplay }}</p> <ul class=tasks> <li><a href=# class="js-zoomto zoomto">{{ model.messages.centerOnLocation }}</a></li> <li><a href=# class="js-deletemarkup deletemarkup">{{ model.messages.deleteMeasurement }}</a></li> </ul> ';
  }
]);
